...
def transform_traj_to_local_coordinate(traj: torch.Tensor, position: torch.Tensor,
...
def transform_traj_to_global_coordinate(traj: torch.Tensor, position: torch.Tensor,
                                        heading: torch.Tensor) -> torch.Tensor:
...
def transform_point_to_local_coordinate(point: torch.Tensor, position: torch.Tensor,
                                        heading: torch.Tensor) -> torch.Tensor:
...
def transform_point_to_global_coordinate(point: torch.Tensor, position: torch.Tensor,
                                         heading: torch.Tensor) -> torch.Tensor:
    rotation_matrix = generate_counterclockwise_rotation_matrix(heading)
    point = torch.matmul(rotation_matrix, point.unsqueeze(-1)).squeeze(-1)
    point = point + position
...
def generate_reachable_matrix(edge_index: torch.Tensor, num_hops: int, max_nodes: int) -> list:
    ...
    for _ in range(num_hops):
        ...
        current_matrix = torch.sparse_coo_tensor(current_matrix.indices(), torch.ones_like(current_matrix.values()),
                                                 current_matrix.size())
...
def generate_predict_mask(visible_mask: torch.Tensor, num_visible_steps: int) -> torch.Tensor:
    ...
    conv_result = torch.nn.functional.conv2d(visible_mask.float().unsqueeze(0).unsqueeze(0),
                                             window.unsqueeze(0).unsqueeze(0))
    conv_result = conv_result.squeeze(0).squeeze(0)

    predict_mask = conv_result == num_visible_steps
    predict_mask = torch.cat(
        [torch.zeros((visible_mask.size(0), num_visible_steps - 1), dtype=torch.bool, device=visible_mask.device),
         predict_mask], dim=1)
